...
from app.outer.interfaces.gateways.client import \
    file_document_atlas_service_client
...
async def do_before_each_tests():
    ...
    for index, file_document in enumerate(file_document_mocks):
        async with await file_document_atlas_service_client.save_one(file_document.dict()) as response:
            assert response.status == 200
            file_document_json = await response.json()
            file_document_entity = FileDocument(**file_document_json)
...
async def do_after_each_tests():
    ...
    for file_document in file_document_mocks:
        async with await file_document_atlas_service_client.delete_one_by_id(file_document.id) as response:
            assert response.status == 200
...
async def test_find_all_file_document():
    async with await file_document_atlas_service_client.find_all() as response:
        assert response.status == 200
...
async def test_save_one_file_document():
    ...
    async with await file_document_atlas_service_client.save_one(
...
async def test_find_one_file_document_by_id():
    ...
    async with await file_document_atlas_service_client.find_one_by_id(
            file_document_id
    ) as response:
        assert response.status == 200
...
async def test_update_one_file_document():
    ...
    async with await file_document_atlas_service_client.update_one_by_id(
...
async def test_delete_one_file_document_by_id():
    ...
    async with await file_document_atlas_service_client.delete_one_by_id(
