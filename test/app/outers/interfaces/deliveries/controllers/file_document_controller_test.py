...
async def test__create_one__should_create_one_file_document__succeed(main_test: MainTest):
    ...
    assert response_body.data.file_extension == file_document_creator_body.file_extension
    assert response_body.data.file_data_hash == hashlib.sha256(file_document_creator_body.file_data).hexdigest()
    assert response_body.data.file_meta == dict()
...
async def test__patch_one_by_id__should_patch_one_file_document__succeed(main_test: MainTest):
    ...
    assert response_body.data.file_extension == file_document_patcher_body.file_extension
    assert response_body.data.file_data_hash == hashlib.sha256(file_document_patcher_body.file_data).hexdigest()
    assert response_body.data.file_meta == dict()
