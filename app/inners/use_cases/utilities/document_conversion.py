...
class DocumentConversion:
    ...
    @Locker.wait_lock
    def corpus_to_pdf(self, process_body: ProcessBody, output_file_path: Path) -> Path:
        if process_body.corpus_source_type == "text":
            result_output_file_path = self.text_to_pdf(
                text=process_body.corpus,
                output_file_path=output_file_path
            )
        elif process_body.corpus_source_type == "file":
            result_output_file_path = self.file_to_pdf(
                input_file_path=Path(process_body.corpus),
                output_file_path=output_file_path
            )
        elif process_body.corpus_source_type == "web":
            result_output_file_path = self.web_to_pdf(
                url=process_body.corpus,
                output_file_path=output_file_path
            )
        else:
            raise ValueError(f"Corpus source type {process_body.corpus_source_type} is not supported.")
