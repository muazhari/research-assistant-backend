...
async def test__refresh_access_token__success(main_context: MainContext) -> None:
    ...
    assert content.data.id == selected_session_mock.id
    assert content.data.account_id == selected_session_mock.account_id
    assert content.data.access_token != selected_session_mock.access_token
    assert content.data.refresh_token == selected_session_mock.refresh_token
    assert content.data.access_token_expired_at > selected_session_mock.access_token_expired_at
    assert content.data.refresh_token_expired_at == selected_session_mock.refresh_token_expired_at
