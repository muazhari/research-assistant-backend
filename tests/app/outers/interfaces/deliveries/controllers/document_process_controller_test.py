...
async def test__find_one_by_id__should__succeed(main_context: MainContext):
    selected_document_process_mock: DocumentProcess = \
        main_context.all_seeder.document_process_seeder.document_process_mock.data[0]
    ...
    assert content.data == selected_document_process_mock
...
async def test__create_one__should_create_one_document_process__succeed(main_context: MainContext):
    ...
    selected_session_mock: Session = main_context.all_seeder.session_seeder.session_mock.data[0]
    selected_document_type_mock_initial = main_context.all_seeder.document_type_seeder.document_type_mock.data[0]
    selected_document_type_mock_final = main_context.all_seeder.document_type_seeder.document_type_mock.data[1]
    ...
    await main_context.all_seeder.up_one_document(selected_document_mock_initial)
    ...
    await main_context.all_seeder.up_one_document(selected_document_mock_final)
...
async def test__patch_one_by_id__should_patch_one_document_process__succeed(main_context: MainContext):
    ...
    selected_document_process_mock: DocumentProcess = \
        main_context.all_seeder.document_process_seeder.document_process_mock.data[0]
    selected_document_mock_initial: Document = main_context.all_seeder.document_seeder.document_mock.data[0]
    selected_document_mock_final: Document = main_context.all_seeder.document_seeder.document_mock.data[1]
    current_time = datetime.now(tz=timezone.utc)
    started_at = current_time + timedelta(minutes=0)
    finished_at = current_time + timedelta(minutes=1)
    ...
    assert content.status_code == status.HTTP_200_OK
    assert content.data.id == selected_document_process_mock.id
...
async def test__delete_one_by_id__should_delete_one_document_process__succeed(main_context: MainContext):
    ...
    assert content.status_code == status.HTTP_200_OK
    assert content.data == selected_document_process_mock
    main_context.all_seeder.delete_many_document_process_by_id_cascade(selected_document_process_mock.id)
